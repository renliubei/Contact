...
public interface PictureInteractiveVideoDao {
    ...
    @SELECT("SELECT * FROM picture_interactive_video WHERE file_name = #{name} AND deleted = false")
    Picture selectPictureByFileName(@Param("name") String fileName);

    @SELECT("SELECT * FROM picture_interactive_video WHERE file_name_by_user = #{name} AND deleted = false")
    List<Picture> selectPictureByName(@Param("name") String nameByUser);

    @INSERT("INSERT INTO picture_interactive_video (file_name, name_by_user, upload_time, url) " +
    ...
    @UPDATE("UPDATE picture_interactive_video SET deleted = true WHERE file_name = #{name}")
